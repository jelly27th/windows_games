/* 
  32-bit polygon transformation demo with matrices
*/
...
int Draw_Line(int x0, int y0, int x1, int y1, DWORD color, 
...
int Game_Main(void* parms = NULL, int num_parms = 0) {
   ...
   DWORD color = _RGB32BIT(0,0,255,0);
   sprintf(buffer,"(x,y) = [%d, %d]     ",ship.x0, ship.y0);
   Draw_Text_GDI(buffer, 8,SCREEN_HEIGHT - 16, color, lpddsback);
   Draw_Text_GDI((char*)"<A> and <S> - Scale    <Z> and <X> - Rotate     <Arrows> - Translate     <ESC> - Exit", 8,8,color, lpddsback);
...
int Game_Init(void* parms = NULL, int num_parms = 0) {
   ...
   ship.xv          = 0;
   ship.yv          = 0;
   ship.color       = _RGB32BIT(255,0,255,0); // green
